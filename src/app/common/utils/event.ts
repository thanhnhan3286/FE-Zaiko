...
const toFull = (set:any):any => (c:any) =>
  set.delta
    ? String.fromCharCode(c.charCodeAt(0) + set.delta)
    : [...set.full][[...set.half].indexOf(c)];
const toHalf = (set:any):any => (c:any) =>
  set.delta
    ? String.fromCharCode(c.charCodeAt(0) - set.delta)
    : [...set.half][[...set.full].indexOf(c)];
const re = (set:any, way:any):any => set[way + 'RE'] || new RegExp('[' + set[way] + ']', 'g');
...
const half1:String[] = ['ｳﾞ', 'ｶﾞ', 'ｷﾞ', 'ｸﾞ', 'ｹﾞ', 'ｺﾞ', 'ｻﾞ', 'ｼﾞ', 'ｽﾞ', 'ｾﾞ', 'ｿﾞ', 'ﾀﾞ', 'ﾁﾞ', 'ﾂﾞ', 'ﾃﾞ', 'ﾄﾞ',
 'ﾊﾞ', 'ﾋﾞ', 'ﾌﾞ', 'ﾍﾞ', 'ﾎﾞ', 'ﾊﾟ', 'ﾋﾟ', 'ﾌﾟ', 'ﾍﾟ', 'ﾎﾟ'];
const full1 = ['ヴ', 'ガ', 'ギ', 'グ', 'ゲ', 'ゴ', 'ザ', 'ジ', 'ズ', 'ゼ', 'ゾ', 'ダ', 'ヂ', 'ヅ', 'デ', 'ド',
'バ', 'ビ', 'ブ', 'ベ', 'ボ', 'パ', 'ピ', 'プ', 'ペ', 'ポ'];
const half2 = ['ｱ', 'ｲ', 'ｳ', 'ｴ', 'ｵ',
'ｶ', 'ｷ', 'ｸ', 'ｹ', 'ｺ',
'ｻ', 'ｼ', 'ｽ', 'ｾ', 'ｿ',
'ﾀ', 'ﾁ', 'ﾂ', 'ﾃ', 'ﾄ',
'ﾅ', 'ﾆ', 'ﾇ', 'ﾈ', 'ﾉ',
...
'ｑ', 'ｒ', 'ｓ', 'ｔ', 'ｕ', 'ｖ', 'ｗ', 'ｘ','ｙ', 'ｚ',
'＜', '＞', '？', '：', '；', '｜', '／'];
const shiftCharCode = (Δ:any) => (c:any):any => String.fromCharCode(c.charCodeAt(0) + Δ);

export const convertHalfToFull = (str0:any):any => {
  for (let i = 0; i < half1.length; i++) {
    str0 = str0.replaceAll(half1[i], full1[i]);
  }

  for (let i = 0; i < half2.length; i++) {
    str0 = str0.replaceAll(half2[i], full2[i]);
  }

  return str0.replace(/[!-~]/g, shiftCharCode(0xFEE0));
};

export const convertFullToHalf = (str0:any):any => {
  for (let i = 0; i < full1.length; i++) {
    str0 = str0.replaceAll(full1[i], half1[i]);
    // str0 = str0.replaceAll('ﾟ','');
  }

  for (let i = 0; i < full2.length; i++) {
    str0 = str0.replaceAll(full2[i], half2[i]);
    // str0 = str0.replaceAll('ﾟ','');
  }

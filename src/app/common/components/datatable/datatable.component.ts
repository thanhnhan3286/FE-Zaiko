...
export class DatatableComponent implements OnChanges, AfterContentChecked, AfterViewChecked {
  ...
  @Input() public isExport!: boolean;
  @Input() public isDisableExport!: boolean;
  @Output() public handleBtnExportPdf: EventEmitter<any> = new EventEmitter<any>();
  ...
  public handelTabindex(): void {
    let data: HTMLCollectionOf<Element> = document.getElementsByClassName('mat-sort-header-container');
    ...
    while (index < data.length) {
      data.item(index)?.setAttribute('tabindex', '-1');
      index++;
    }
  ...
  public onBtnExportPdf():void {
    this.handleBtnExportPdf.emit();
  }

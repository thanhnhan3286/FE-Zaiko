...
export class GroupSearchComponent implements OnChanges, OnDestroy {
  ...
  public ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['searchConfig'].currentValue) {
      ...
      // document.title = '在庫管理システム'+ '（'+ this.languageService.get(
      //   (isUndefined(this.searchConfig.title3) || isEmpty(this.searchConfig.title3) ?
      //  (isUndefined(this.searchConfig.title2) || isEmpty(this.searchConfig.title2) ? this.searchConfig.title : this.searchConfig.title2) : this.searchConfig.title3)?.toString() as string)+'）' ;
            ...
            if (el.type.includes('input-number-from-to')) {
              ...
              if (el.type === 'select') {
                ...
                if (el.type === 'select-year') {
                  ...
                  widthRow += (el.width || 0);
                }else if(el.type.includes('checkbox')) {
                    el.checkBoxList?.map(x=>{
                      group[x.id] = !isNull(x.valueDefault) || !isUndefined(el.defaultValue) ? new FormControl(x.valueDefault, []) : new FormControl(false, []);
                      this.dataSearchInit[x.id] = !isNull(x.valueDefault) || !isUndefined(x.valueDefault) ? x.valueDefault : false;
                    });
                    widthRow += (el.width || 0);
                } else
  ...
  public onBtnExportCSVClk(): void {
    this.btnSearchClicked.emit({ action: 'csv', form: this.searchGroup });
  }
  ...
  public onBtnClearClk(): void {
    ...
    this.searchConfig.config.map((el) => {
      if (el.type === 'radio' && el.defaultValue) {
        ...
        this.searchGroup.controls[el.id + 'From']?.setValue(el.defaultValue);
        this.searchGroup.controls[el.id + 'To']?.setValue(el.defaultValue);

        if (!isUndefined(el.isClearDropList) && el.isClearDropList === false) {
          el.dataFrom = clone(el.dataFrom);
          el.dataTo = clone(el.dataTo);
        }

        if (!isUndefined(el.isClearDropList) && el.isClearDropList === true) {
          el.dataFrom = clone([]);
          el.dataTo = clone([]);
        }
      }
  ...
  public handleNgSelectChange(event: any, formcontrolName: string): void {
    let selectSeachData: SelectSearchModel = {
      fromControlName: formcontrolName,
      value: event
    };

...
import { HttpClientResponse, HttpClientResponseList } from '@core/models/http-response.model';
import { Observable, of } from 'rxjs';
import { catchError, tap } from 'rxjs/operators';
import { HttpOutputListResponse } from 'src/app/pages/output/list/model/output-list.model';
...
export abstract class HttpService {
  ...
  public getParamList(url: string, param: HttpParams): Observable<HttpClientResponseList> {
    return this.http.get<HttpClientResponse>(url, { params: param }).pipe(
      tap((response) => response),
      catchError((err) => of(err))
    );
  }

  public getParam(url: string, param: HttpParams): Observable<HttpClientResponse> {
    return this.http.get<HttpClientResponse>(url, { params: param }).pipe(
      tap((response) => response),
      catchError((err) => of(err))
    );
